...
left_image = cv2.imread("Input/im2.png", cv2.IMREAD_GRAYSCALE)
right_image = cv2.imread("Input/im6.png", cv2.IMREAD_GRAYSCALE)
...
if left_image is None or right_image is None:
    ...
    for y in tqdm(
        range(half_window, left_image.shape[0] - half_window), desc="Processing Rows"
    ):
        for x in tqdm(
            range(half_window + max_disparity, left_image.shape[1] - half_window),
            leave=True,
        ):
            left_patch_x = grad_x_left[
                y - half_window : y + half_window + 1,
                x - half_window : x + half_window + 1,
            ]
            left_patch_y = grad_y_left[
                y - half_window : y + half_window + 1,
                x - half_window : x + half_window + 1,
            ]
            ...
            for d in range(max_disparity):
                ...
                right_patch_x = grad_x_right[
                    y - half_window : y + half_window + 1,
                    right_x - half_window : right_x + half_window + 1,
                ]
                right_patch_y = grad_y_right[
                    y - half_window : y + half_window + 1,
                    right_x - half_window : right_x + half_window + 1,
                ]

                cost = np.sum(
                    np.abs(left_patch_x - right_patch_x)
