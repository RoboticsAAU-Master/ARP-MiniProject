...
cap_left = cv2.VideoCapture("Input/3D_L4948.MP4")
cap_right = cv2.VideoCapture("Input/3D_R0219.MP4")
...
def pool_expand(arr, kernel_size):
    ...
    expanded_pooled = np.repeat(
        np.repeat(pooled, kernel_size, axis=0), kernel_size, axis=1
    )
...
for f in tqdm(range(int(min_length - 1))):
    ...
    disparity_map = sm.calc_disparity(
        frame_gray_l, frame_gray_r, num_disparities, window_size
    )
    disparity_map = pool_expand(disparity_map, 1)[
        :, num_disparities + window_size : disparity_map.shape[1]
    ]
